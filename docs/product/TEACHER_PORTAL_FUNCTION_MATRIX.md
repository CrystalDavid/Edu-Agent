...
> 当前基线：Gate 2.7 已 verified；当前分支进行 Gate 2.8 日程、待办与教师统一工作台。
...
|---|---|---|---|---|---|
| 概览 | 今日 TeacherTodo/CalendarEvent、来源 action items、待批改、待审核计划、未完成备课、模型失败、最近课时/文件均来自 PostgreSQL；可创建个人 Todo | 备课组动态、学校动态仍来自前端数组 | 所有 Mock 区均明确标注只读演示 | 制作课件、协作写操作明确禁用 | — |
| 日程 | TeacherTodo CRUD/状态/偏好/资源关联、CalendarEvent CRUD/状态、Todo 安排日历、来源业务投影、日/周/月统一 API、URL view/date 恢复、source snooze/restore 全部为真实数据 | 当前筛选和未保存 Modal 表单仅为视图状态 | 来源业务日历项只读，进入源页面修改 | 外部/共享日历、复杂重复日程、自动完成源事项未实现 | — |
...
Gate 2.5–2.5C 备课/文件链、Gate 2.6A 模型执行和 Gate 2.7 作业/Evidence 闭环继续为 `REAL`。Gate 2.8 的个人 Todo、手工 Calendar、来源业务投影、提醒偏好和 Todo→Agent handoff 均接入 Product Composition Root；默认测试仍用 Mock/Fake，不发起公网请求。考试和开放 Agent 对话仍明确为 Mock/READ_ONLY。`DEAD = 0`。
