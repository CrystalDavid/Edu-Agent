...
> 当前基线：Gate 2.6A 已验证；Gate 2.5B 功能分支待人工验收。
...
|---|---|---|---|---|---|
| 概览 | 未完成备课 Task、待审核计划、已准备未完成 Task、最近课时和最近教学文件来自 PostgreSQL；点击进入真实 Task/File | 学生概况、备课组动态、学校动态和非备课快捷信息仍来自前端数组 | — | — | — |
| 日程 | — | 日/周/月切换、待办勾选、转日程草稿，仅 React state | — | 通用 Calendar/Todo 写入未实现 | — |
| 教学 / 课程 | CourseRun → CurriculumUnit → Lesson、教学目标、备课状态、计划、关联 Task，以及关联参考文件/正式 DOCX 均为真实数据 | 当前选中的 Unit/Lesson 是页面视图状态，刷新后回到默认选中项 | — | 课程 CRUD 未实现 | — |
...
| 文件 | PostgreSQL FileAsset/FileVersion/Binding；LocalObjectStore 字节；上传、列表、搜索、分类、排序、下载、图片/PDF/文本预览、详情、新版本、版本历史、软删除/恢复，以及所选 Lesson、备课 Task、current approved TeachingPlan Revision 关联；DOCX/PPTX/XLSX 在服务端有限解压并提取安全文本摘要 | 页面筛选和当前选择为临时视图状态 | Office 文件显示文件信息、本地提取摘要与下载，不做完整浏览器渲染 | 在线新建、分享、协作、云同步明确禁用；正式成果删除被拒绝 | — |
...
| Teaching Plan | current approved、active in-review、draft、superseded、history、继续审阅、独立批准、返回 Task、显式完成；明确 approved Revision 导出 DOCX、查看/下载版本 | — | 历史 immutable Revision 只读 | draft/in-review 正式导出、approved 原地编辑和无 approved plan 的完成命令被拒绝 | — |
...
Gate 2.5 业务闭环继续全部为 `REAL`。Gate 2.6A 模型执行与安全摘要为 `REAL`。Gate 2.5B 的上传、版本、绑定、DOCX 导出、下载和页面联动也已接入正式 Product Composition Root；默认测试仍用 Mock/Fake，不发起公网请求。`DEAD = 0`。
...
- 作业/测试筛选和演示操作；
- 学生页交互；
- 文件筛选、当前选择和预览滚动位置（文件业务数据与字节不会丢失）；
- 通用 `/agent` 对话和上下文；
- 设置页组件状态。
