...
|---|---|---|---|---|---|
| 概览 | 教师身份和启动 workspace 来自产品 API | 待办、课程、学生概况、动态、最近文件 | — | — | — |
| 日程 | — | 日/周/月切换、待办勾选、转日程草稿，仅 React state | — | — | 刷新后全部恢复初始数组 |
| 教学 / 课程 | “生成新版本”进入真实 Copilot；TeachingPlan 历史入口真实 | 课程树、章节、文件预览 | 演示课程与材料摘要 | 编辑、下载等未接后端 | 非真实按钮会明确提示“高保真界面预览”，不伪装持久化 |
...
| Copilot 详情 | Task request、Proposal、策略、diff、Evidence、Disposition、直接 URL 恢复 | MockModelProvider 的两套策略内容是确定性固定模板 | 本地演示身份与模型边界提示 | 已处置 Proposal 禁止再次处置 | 无 API 失败 fallback；失败显示真实错误 |
| Teaching Plan | current approved、current in-review、draft list、history、独立批准 | — | 历史 Revision 只读 | approved Revision 原地编辑被数据库拒绝 | — |
| Runs | Task/Run/Contract/ContextManifest/请求/Evidence/Authorization/ModelExecution/Audit/Outbox | — | 审计与运行解释只读 | 不显示隐藏思维链或 secret | — |
