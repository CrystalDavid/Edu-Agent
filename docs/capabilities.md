...
| Agent / Copilot | REAL | 封存 TaskWorkingSet/授权上下文；在同一备课 Conversation 中持久化教师请求、安全结果摘要和显式命令回执；用可重建 WorkingMemory 理解延续要求；受控 Catalog 支持明确 remember/forget；Lesson Preparation 按 Scope 解析长期偏好，并把“这次/本次”低风险要求作为 current-conversation typed override 临时 replace/suppress，clear/close/到期或新 Conversation 后失效；Proposal 与 Runs 显示本次参考与覆盖原因；刷新或服务重启后恢复 | Work Conversation/Turn + Runtime WorkingMemorySnapshot V1/V2、MemoryContextPackManifest V1/V2/V3 + Personalization scoped Preference/application/outcome + Capability/Artifact PostgreSQL | 2.4、2.5、2.6A；记忆升级 M1 + M0-lite + M2A–M2C2（均未标记 Verified Gate） | “本次参考”只证明平台选入上下文，不证明模型完整采用；临时覆盖不修改长期 Preference/epoch 且不跨 Conversation；显式命令不是通用自然语言学习或物理删除；无自动习惯学习或向量检索 |
...
| 教师偏好与个性化 | REAL（最小） | 候选 draft、教师确认/修改/拒绝/撤销、不可变 revision、valid time、global/subject/subject-grade/CourseRun/lesson/task 领域 Scope、Skill constraint 与 teacherMemoryEpoch；Lesson Preparation 按确定性作用域优先级解析；明确低风险 Catalog 项可直接确认，重复不增 epoch、冲突需老师替换/保留；显式 Forget 的唯一匹配立即 revoke，多匹配封存选项并由老师确认，后续新 Context 不再选取 | 设置页当前只暴露 global/CourseRun；显式 remember/forget 只支持服务器 Catalog 与 global/当前 CourseRun；Forget 不删除 Revision/Audit/历史 Run；temporary override 由 Runtime 拥有，不写 Personalization；其他 Skill 兼容路径只读有效 global、无 Skill 限制偏好；无自由文本记忆、自动 Candidate、向量检索或未确认 Memory 入模 |
| 备课会话工作记忆 | REAL（M1 + M2） | owner-scoped Conversation/不可变 Turn、最多六条近期教师要求、当前目标、指代和最近安全结果引用；V2 Snapshot 另保存最多 10 条受控 current-conversation typed override，支持 replace、suppress、同 key supersede、单项/全部 clear；remember/forget command 与 receipt 不污染 active goal；刷新/重启恢复，close/到期或 Task terminal 后排除且新 Conversation 不继承 | 临时覆盖只接入 Lesson Preparation，不修改长期 Preference/version/epoch，不 eligible for consolidation；Task reopen 不复活旧 Conversation；不保存原始供应商响应、完整 Prompt 或隐藏推理；无语义向量检索或跨任务行为学习；正式 retention 期限待产品确认 |
| 教师记忆应用观测 | REAL（M0-lite + V3） | Runtime 保留历史 Pack V1/V2，并为 temporary-aware `lesson-preparation@7` 封存 Pack V3（WorkingMemory V2 ref/hash、override set hash、最小 decision 与 injected/suppressed 计数）；同 key durable Preference 记录 `overridden/current_instruction_override`；Provider retry/repair 复用 sealed state；Personalization application/outcome 仍只记录 durable Preference | temporary override 自身不创建 durable application row；不复制 Turn、WorkingMemory、Preference value、完整 Prompt/响应/Evidence；“参考”不等于模型遵循；生产 feature flag fail closed；正式 retention 待产品确认 |
...
记忆系统把“当前正在做什么”“当前 Conversation 的临时要求”和“老师长期习惯”正式拆开：Work Schema 保存备课 Conversation/不可变 Turn，Runtime Schema 保存可由 Turn 重建的 WorkingMemorySnapshot V1/V2。`lesson-preparation@7` 在保持当前请求最高优先级的同时，读取 V2 typed override、调用 scoped Preference resolver 并封存 Pack V3；`@1–@6` 与 Pack V1/V2 继续用于历史恢复或 feature flag 兼容。临时要求不会自动晋升为 TeacherPreference。
...
PR-2B 为 Lesson Preparation Conversation 增加 `teacher-preference-catalog@1` 与纯确定性 `explicit-memory-command-interpreter@1`。只有 owning teacher 的明确 durable marker、完整白名单解析、低风险 canonical value、获权 global/当前 CourseRun Scope 和开启的服务端 flag 才能直接创建 Candidate + confirmed Preference；重复命令不增加 epoch，同 key/Scope 冲突只创建 draft Candidate，必须由老师选择替换或保留。

PR-2C1 增加独立纯确定性的 `explicit-forget-command-interpreter@1`。唯一 active matching Preference 在一个 Personalization 事务中 revoke；未声明 Scope、“全部”或其他多匹配先把最多 10 个 refs 封存到 immutable receipt，老师显式选择后才撤销。成功后 epoch 按实际 mutation 数增加，新 Resolver/Pack/ModelExecution/application 不再使用，历史 Run 仍显示当时 revision 和当前已撤销。command/receipt 不创建 ModelExecution/Proposal，也不改变 WorkingMemory active goal。

PR-2C2 增加纯确定性的 `temporary-preference-override-interpreter@1`。带“这次/本次”marker 且被受控 Catalog 完整解析的低风险子句进入 Runtime-owned WorkingMemory V2；replace/suppress 在当前 Conversation 后续 Turn 延续，clear/close/到期或新 Conversation 后失效。Runtime 在 durable resolver 后按 canonical key 覆盖，当前请求仍最高优先；长期 Preference、revision 和 `teacherMemoryEpoch` 保持不变，typed override 固定 `eligibleForConsolidation=false`。普通行为自动学习和 M3–M5 仍未实现。

PR-2A 增加的是结构化长期偏好作用范围，而不是自动记忆学习：同一 canonical key 可在 global 与不同 CourseRun 等 Scope 中同时 active，Resolver 使用确定性优先级选择最具体项，`teacherMemoryEpoch` 在确认、值/Scope/valid time 更新和撤销时递增。Scope 只描述适用性，每次运行和写入仍重新执行 Session、ActingContext、purpose、CourseRun/Task 与 field-mask 授权。当前 remember/forget/temporary override 仍局限于受控 Catalog 和 Lesson Preparation；行为 Observation、Candidate 自动提炼、Episode/Habit、全文或向量检索仍未实现。
