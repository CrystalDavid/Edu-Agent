...
- 确认 49 个只向前 Migration、开发数据库和 LocalObjectStore 均未被改变；
...
- **M2（未实现）：**自然语言“记住 / 忘掉 / 仅本次”与明确 scope、consent、valid time、teacher memory epoch 和撤销语义；
...
Candidate 在教师确认前不得进入正式 Context；M2–M5 不得被当前 M1/M0-lite 表、界面文案或文档描述成已实现。
