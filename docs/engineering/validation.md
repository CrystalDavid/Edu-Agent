...
- 默认 Playwright E2E control endpoint 只在隔离测试 server 中支持重启 API，并可用 `explicit-remember=enabled|disabled|default`、`explicit-forget=enabled|disabled|default` 和 `temporary-overrides=enabled|disabled|default` 验证服务端 flag；该 endpoint 不注册到产品 API；
